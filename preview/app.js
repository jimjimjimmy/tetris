...
function makeInitState2P() {
  ...
  return {
    ...
    resolveAt: null,
    // ms when the first death occurred (grace anchor)
...
function TetrisGame2P() {
  ...
  React.useEffect(() => {
    ...
    const id = setTimeout(() => {
      setState(s => s.phase === "countdown" ? {
        ...s,
        countdownLeft: s.countdownLeft - 1
      } : s);
    }, 1000);
  ...
  p1WinsRef.current = p1Wins;
  p2WinsRef.current = p2Wins;
  ...
  const connectToRoom = (code, opts = {}) => {
    ...
    ws.onmessage = e => {
      ...
      if (msg.type === "role") {
        ...
        setState(() => ({
          ...buildFreshOnline(msg.seed, onlineRoleRef.current),
          phase: "countdown",
          countdownLeft: COUNTDOWN_SECS
        }));
        ...
        setState(s => s.phase === "playing" || s.phase === "over" || s.phase === "countdown" ? {
          ...s,
          opponentLeft: true
        } : s);
  ...
  if (phase === "start") {
    ...
    if (startTab === "2players") {
      ...
      }, "Share your code"), /*#__PURE__*/React.createElement("span", {
        style: {
          marginRight: "-3px"
        }
      }, "with opponent"))), /*#__PURE__*/React.createElement("div", {
        style: {
          position: "absolute",
          left: 0,
          right: 0,
