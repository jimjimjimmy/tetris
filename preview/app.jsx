...
const driftActs = { tap: 0, left: 0, right: 0, up: 0, down: 0, soft: 0 };
...
const AI_LEVEL_CONFIG  = {
  ...
  1: { randomRatio: 0.70, aiPeriod: 1, holePenalty: 8,   bumpPenalty: 0.5, maxHPenalty: 1.5, coverageBonus: 10, linePts: 100,  tickMs: 900  },
  2: { randomRatio: 0.40, aiPeriod: 1, holePenalty: 20,  bumpPenalty: 1.5, maxHPenalty: 3,   coverageBonus: 16, linePts: 300,  tickMs: 700  },
  3: { randomRatio: 0.10, aiPeriod: 1, holePenalty: 40,  bumpPenalty: 2.5, maxHPenalty: 5,   coverageBonus: 24, linePts: 500,  tickMs: 500  },
  4: { randomRatio: 0.02, aiPeriod: 1, holePenalty: 70,  bumpPenalty: 4,   maxHPenalty: 7,   coverageBonus: 32, linePts: 800,  tickMs: 320  },
...
const APP_COMMIT     = "21eb754";
const APP_BUILD_DATE = "2026-08-02T02:59:41";
...
const netPieces = { online: false, _r1: null, _r2: null };
...
function shiftBoundary2P(g, n1, n2, wrap) {
  ...
  if (newBdy <= 0) { newBdy = 0; winner = 1; phase = "over"; }
  else if (newBdy >= ROWS_2P) { newBdy = ROWS_2P; winner = 2; phase = "over"; }
  ...
  if (newBdy !== boundary) {
    ...
    if (delta > 0) {
      ...
      for (let r = boundary; r < ROWS_2P; r++) for (let c = 0; c < COLS; c++) {
        if (board[r][c] === CELL_P1) { const nr = r - delta; if (nr >= 0 && nr < ROWS_2P) next[nr][c] = CELL_P1; }
      }
    } else {
      const shift = -delta;
      ...
      for (let r = 0; r < boundary; r++) for (let c = 0; c < COLS; c++) {
        if (board[r][c] === CELL_P2) { const nr = r + shift; if (nr >= 0 && nr < ROWS_2P) next[nr][c] = CELL_P2; }
      }
    }
    board = next;
  ...
  if (realShift > 0) lastGain = { player: 1, rows: realShift, ts: Date.now() };
  else if (realShift < 0) lastGain = { player: 2, rows: -realShift, ts: Date.now() };

  return { board, boundary: newBdy, p1, p1Next, p1NextNext, p2, p2Next, p2NextNext, winner, phase, lastGain };
}
...
function makeInitState2P() {
  ...
  return {
    ...
    online:       false,    // true when this is a networked match (no AI)
    onlineStatus: "idle",   // "idle" | "waiting" | "ready" | "disconnected"
    onlineRole:   null,     // "p1" | "p2" | null (assigned by server)
    opponentLeft: false,    // true mid-game when opponent disconnects
    netResult:    null,     // "win" | "lose" | null -- online game-over outcome
    // Sync stamps: counters the tick bumps so broadcast effects fire.
    lockStamp:    0,        // ++ when LOCAL player locks (triggers "lk" send)
    lockLines:    0,        // line count from the most recent local lock
    lockBoard:    null,     // pre-shift (post-clear) board snapshot at local lock
    goStamp:      0,        // ++ when LOCAL player tops out (triggers "go" send)
...
function BgVignette() {
  return (
    <svg width="402" height="874" viewBox="0 0 402 874" fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{position:"absolute",inset:0,pointerEvents:"none",width:"100%",height:"100%"}}>
      <rect opacity="0.5" y="0.5" width="402" height="874" fill="url(#bv_g0)"/>
      <rect opacity="0.5" y="0.5" width="402" height="874" fill="url(#bv_g1)"/>
      <rect opacity="0.3" y="0.5" width="401" height="874" fill="black"/>
      <defs>
        <radialGradient id="bv_g0" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
          gradientTransform="translate(201 211.391) rotate(90) scale(663.109 663.109)">
          <stop stopColor="#111213"/>
          <stop offset="1" stopColor="#111213" stopOpacity="0"/>
        </radialGradient>
        <radialGradient id="bv_g1" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
...
const ArrowR = ({opacity=1}) => (
  <svg width="11.693" height="10.182" viewBox="0 0 11.6932 10.182" fill="none"
    style={{display:"block",flexShrink:0,opacity}} xmlns="http://www.w3.org/2000/svg">
    <path d="M6.60227 10.1818L5.72727 9.31818L9.32955 5.71591H0V4.46591H9.32955L5.72727 0.875L6.60227 0L11.6932 5.09091L6.60227 10.1818Z" fill="white"/>
  </svg>
);
const ArrowL = ({opacity=1}) => (
  <svg width="11.693" height="10.182" viewBox="0 0 11.6932 10.182" fill="none"
    style={{display:"block",flexShrink:0,opacity}} xmlns="http://www.w3.org/2000/svg">
    <path d="M5.09091 10.1818L0 5.09091L5.09091 0L5.96591 0.863636L2.36364 4.46591H11.6932V5.71591H2.36364L5.96591 9.30682L5.09091 10.1818Z" fill="white"/>
  </svg>
);
...
const haptic = {
  ...
  _impact(baseStyle, vibrateMs) {
    if (!this._s.haptics) { console.log("[haptic] skip -- haptics off"); return; }
    const H = window.Capacitor?.Plugins?.Haptics;
    const scaled = this._scale(baseStyle);
    console.log("[haptic] impact", scaled, "| H:", !!H, "| intensity:", this._s.hapticIntensity);
    if (H) {
      try { H.impact({ style: scaled }); } catch (e) { console.error("[haptic] impact failed:", e); }
      return;
    }
  ...
  light() {
    ...
    if (H) {
      ...
      try { H.impact({ style: this._scale("LIGHT") }); } catch (e) { console.error("[haptic] light failed:", e); }
      return;
    }
    const d = this._s.hapticIntensity;
    if (typeof navigator?.vibrate === "function") {
...
const sfx = {
  ...
  _s:        { soundFX: true, volume: 4 },
  _attempted: false,
  _ctx:      null,   // shared AudioContext (SFX + BGM)
  _bufs:     {},     // key -> AudioBuffer
  _bgmGain:  null,   // GainNode for BGM (volume control + looping)
  _bgmSrc:   null,   // AudioBufferSourceNode currently playing BGM

  configure(settings) { this._s = settings; },

  // Diagnostic: call to log sfx + haptic health into Xcode console.
  diagnose() {
    const H = window.Capacitor?.Plugins?.Haptics;
    console.log("[diag] soundFX:", this._s.soundFX, "| volume:", this._s.volume,
      "| Web Audio ctx:", this._ctx ? this._ctx.state : "none",
      "| bufs loaded:", Object.keys(this._bufs).length,
  ...
  preload() {
    ...
    try {
      ...
      this._bgmGain.gain.value = 0.84375; // base BGM gain (+50% x3 from 0.25)
    ...
    Object.entries(files).forEach(function(entry) {
      ...
      fetch(url)
        ...
        .catch(function(e) { console.warn("[sfx] load failed:", key, e.message || e); });
    });
  ...
  _play(key) {
    ...
    var vol = Math.max(0.05, this._s.volume / 10 * 3.375); // +50% x3 (3.375x of base)
    try {
    ...
    } catch(e) { console.warn("[sfx] play error:", key, e); }
  },

  pieceLock()     { this._play("pieceLock"); },
  lineClear(n)    { this._play(n > 1 ? "lineClearMulti" : "lineClearSingle"); },
  ...
  bgmPlay(volume) {
    if (!this._ctx || !this._bufs["bgm"]) { console.warn("[sfx] bgm not ready"); return; }
    if (this._ctx.state === "suspended") { this._ctx.resume().catch(function() {}); }
    if (this._bgmSrc) {
    ...
    this._bgmGain.gain.value = Math.max(0.05, (volume || 1) * 0.84375); // +50% x3
    src.start(0);
    this._bgmSrc = src;
  },
  bgmPause() {
    if (this._ctx && this._ctx.state === "running") { this._ctx.suspend().catch(function() {}); }
  },
  bgmStop() {
    if (this._bgmSrc) {
      try { this._bgmSrc.stop(); } catch(_) {}
  ...
  bgmVolume(v) {
    if (this._bgmGain) this._bgmGain.gain.value = Math.max(0.05, v * 0.84375); // +50% x3
  },
};
...
function loadSettings() {
  const defaults = {
    soundFX: true, volume: 4,
    haptics: true, hapticIntensity: "medium",
    level: AI_LEVEL_DEFAULT, direction: 1,
    drift: false,
  };
...
function SettingsScreen({ settings, onUpdate, onClose, initialSection, exiting }) {
  const inter = "'Inter', sans-serif";

  const secLabel = {
    fontFamily: inter, fontSize: 12, fontWeight: 600,
    letterSpacing: "3.6px", color: "#fff",
    textTransform: "uppercase",
  };
  const rowLbl = {
    fontFamily: inter, fontSize: 10, fontWeight: 400,
    letterSpacing: "3px", color: "#fff", opacity: 0.5,
    textTransform: "uppercase", flex: "1 0 0", minWidth: 0,
  };
  const opt = (active) => ({
  ...
  return (
    <div style={{
      ...
      {/* Title -- Figma 324:6587, y:106 (aligned with Instructions title) */}
      <div style={{
        position: "absolute", left: 44, top: 106,
      ...
      <div style={{
        ...
        {/* SOUND section */}
        <div style={{ display: "flex", flexDirection: "column", ...di(1) }}>
          <div style={{...secLabel, marginBottom: 8}}>Sound</div>

          {/* Sound FX */}
          <div style={{ display: "flex", alignItems: "center", minHeight: 44, gap: 16 }}>
          ...
          {/* Volume 1-5 (Music folded into Sound Fx; gated on soundFX) */}
          <div style={{ display: "flex", alignItems: "center", minHeight: 44, gap: 40, opacity: settings.soundFX ? 1 : 0.3 }}>
        ...
        {/* HAPTICS section */}
        <div style={{ display: "flex", flexDirection: "column", ...di(2) }}>
          <div style={{...secLabel, marginBottom: 8}}>Haptics</div>
          ...
          {/* Intensity */}
          <div style={{ display: "flex", alignItems: "stretch", minHeight: 44, gap: 32, opacity: settings.haptics ? 1 : 0.3 }}>
            <div style={{...rowLbl, display:"flex", alignItems:"center"}}>Intensity</div>
            {[["light","Low"],["medium","Mid"],["strong","High"]].map(([val, label]) => (
              <div key={val}
        ...
        {/* GAME section */}
        <div style={{ display: "flex", flexDirection: "column", ...di(3) }}>
          <div style={{...secLabel, marginBottom: 8}}>Game</div>

          {/* Level */}
          <div style={{ display: "flex", alignItems: "center", minHeight: 44, gap: 40 }}>
          ...
          {/* Direction */}
          <div style={{ display: "flex", alignItems: "center", minHeight: 44, gap: 32 }}>
            <div style={rowLbl}>Direction</div>
            {[{s:1, label:"↑ Up"}, {s:2, label:"↓Dn"}].map(({s, label}) => (
              <div key={s}
          ...
          {/* Drift (solo-only prototype: whole board scrolls + wraps at the edges) */}
          <div style={{ display: "flex", alignItems: "center", minHeight: 44, gap: 16 }}>
            <div style={rowLbl}>Drift</div>
            {[false, true].map(v => (
              <div key={String(v)}
      ...
      {/* Cancel / Done -- Figma y:726 */}
      <div style={{
        position: "absolute", left: 0, right: 0, top: 709,
        ...
        <div
          ...
          style={{
            ...
            letterSpacing: "6px", color: "#fff", textTransform: "uppercase",
            opacity: 0.3, cursor: "pointer", userSelect: "none",
            padding: "17px 0",
          }}
        >Cancel</div>
        <div
...
function InstructionsScreen({ onClose, exiting }) {
  const inter = "'Inter', sans-serif";
  const secLabel = {
    fontFamily: inter, fontSize: 12, fontWeight: 600,
    letterSpacing: "3.6px", color: "#fff", textTransform: "uppercase",
  };
  const body = {
    fontFamily: inter, fontSize: 12, fontWeight: 400,
    lineHeight: 1.3, color: "#fff", opacity: 0.5, margin: 0,
  };
  ...
  return (
    <div style={{
      ...
      {/* Title -- Figma 358:6532, y:106 (aligned with Settings title) */}
      <div style={{
        position: "absolute", left: 44, top: 106,
        fontFamily: inter, fontSize: 16, fontWeight: 500,
        letterSpacing: "8px", textTransform: "uppercase",
      ...
      {/* Content -- Figma 358:6484, x:44 y:165 w:314, column gap 24 */}
      <div style={{
        position: "absolute", left: 44, top: 165, width: 314,
        display: "flex", flexDirection: "column", gap: 24,
      }}>
        <div style={section(1)}>
          <div style={secLabel}>The Board</div>
          <p style={body}>Two players share one board, divided by a boundary line in the middle. You play one side, your opponent the other. Your pieces travel toward the boundary - if you're on the bottom they float up, if you're on top they fall down.</p>
        </div>

        <div style={section(2)}>
          <div style={secLabel}>Pick Your Side (Solo)</div>
          <ul style={bulletUl}>
            <li>UP - play from the bottom, pieces float up.</li>
            <li>DN - play from the top, pieces fall down.<br />The AI takes the opposite side. (In 2-player, sides are assigned when you connect.)</li>
          </ul>
        </div>

        <div style={section(3)}>
          <div style={secLabel}>Controls</div>
          <ul style={bulletUl}>
            <li>Swipe left / right to move.</li>
            <li>Tap to rotate.</li>
            <li>Swipe the way your pieces travel to hard-drop - up if they float up, down if they fall down.</li>
          </ul>
        </div>

        <div style={section(4)}>
          <div style={secLabel}>Claim Territory</div>
          <p style={body}>Clear a row to push the boundary into your opponent's half. Let rows pile up and they'll push it back into yours.</p>
        </div>

        <div style={section(5)}>
          <div style={secLabel}>Winning</div>
          <p style={body}>Two ways to win: push the boundary all the way into their half to claim their whole territory, or force them out when their side fills to the edge. If your side fills up first, you lose.</p>
        </div>

        <div style={section(6)}>
          <div style={secLabel}>Modes</div>
          <ul style={bulletUl}>
            <li>Solo - play an AI opponent; pick your difficulty (1-5).</li>
            <li>2 Players - share your room code with a friend, or join theirs with a code.</li>
          </ul>
        </div>

        {/* Done -- Figma 362:6575, centered */}
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", ...di(7) }}>
          <div
            onPointerDown={onClose}
            onTouchStart={e => e.stopPropagation()}
            style={{
              fontFamily: inter, fontSize: 12, fontWeight: 800,
              letterSpacing: "6px", color: "#fff", textTransform: "uppercase",
              cursor: "pointer", userSelect: "none", padding: "17px 0",
            }}
          >Done</div>
        </div>
      </div>
...
function writeDriftProbe(snap) {
  ...
  const a = driftActs;
  const s = snap || {};
  driftProbeEl.textContent =
    "DRIFT;phase=" + (s.phase || "") +
    ";side=" + (s.playerSide == null ? "0" : s.playerSide) +
    ";p1x=" + (s.p1x != null ? s.p1x : "-") +
    ";p1rot=" + (s.p1rot != null ? s.p1rot : "-") +
    ";p2x=" + (s.p2x != null ? s.p2x : "-") +
    ";p2rot=" + (s.p2rot != null ? s.p2rot : "-") +
    ";p1t=" + (s.p1type || "-") +
    ";p2t=" + (s.p2type || "-") +
    ";p1n=" + (s.p1n || "-") +
    ";p2n=" + (s.p2n || "-") +
    ";p1c=" + (s.p1c != null ? s.p1c : "-") +
    ";p2c=" + (s.p2c != null ? s.p2c : "-") +
    ";bdy=" + (s.boundary != null ? s.boundary : "-") +
    ";l1=" + (s.p1Lines != null ? s.p1Lines : "-") +
    ";l2=" + (s.p2Lines != null ? s.p2Lines : "-") +
    ";set=" + (s.showSettings ? "1" : "0") +
    ";lvl=" + (s.level != null ? s.level : "-") +
    ";sfx=" + (s.sfxOn ? "1" : "0") +
    ";hap=" + (s.hapOn ? "1" : "0") +
    ";vol=" + (s.vol != null ? s.vol : "-") +
    ";paused=" + (s.paused ? "1" : "0") +
    ";over=" + (s.phase === "over" ? "1" : "0") +
    ";tap=" + a.tap + ";left=" + a.left + ";right=" + a.right +
    ";up=" + a.up + ";down=" + a.down + ";soft=" + a.soft;
}
...
function TetrisGame2P() {
  ...
  const slideAnim = navExiting
    ? { animation: `pageOutRight ${NAV_MS}ms ${EASE} both`, pointerEvents: "none" }
    : { animation: `pageInRight ${NAV_MS}ms ${EASE} both` };
  const fadeAnim = navExiting
    ? { animation: `fadeOut 0.24s ease both`, pointerEvents: "none" }
  ...
  React.useEffect(() => {
    const id = setInterval(() => {
      setState(s => {
          ...
          const scoreP1 = (type, rot, x) => {
            ...
            for (let c = 0; c < COLS; c++) {
              ...
              for (let r = ROWS_2P - 1; r >= boundary; r--) {
                if (simAfter[r][c] !== CELL_EMPTY) seen = true;
                else if (seen) holes++;
              }
            }
            ...
            let bumpiness = 0, maxH = 0, coverage = 0;
            for (let c = 0; c < COLS; c++) {
              if (colH[c] > 0) { coverage++; if (colH[c] > maxH) maxH = colH[c]; }
        ...
        if (isValid2P(wrapCells(getCells(p1.type, p1.rot, p1.x, p1.y - 1)), board, boundary, 1)) {
          ...
          if (p1.lockPendingTs == null) {
            ...
            if (!isValid2P(getCells(p1.type, p1.rot, p1.x, p1.y), board, boundary, 1)) {
              ...
              if (s.online) return { ...s, board, p1, p1Next, p1NextNext, p2, p2Next, p2NextNext, iDied: true, resolveAt: s.resolveAt || Date.now(), goStamp: (s.goStamp || 0) + 1 };
        ...
        return {
          ...s,
          board: sh.board, aiTick,
          p1: sh.p1, p1Next: sh.p1Next, p1NextNext: sh.p1NextNext,
          p2: sh.p2, p2Next: sh.p2Next, p2NextNext: sh.p2NextNext,
          phase: sh.phase,
          p1Lines: p1Lines + n1, p2Lines: p2Lines + n2,
          boundary: sh.boundary, winner: sh.winner, lastGain: sh.lastGain,
  ...
  React.useEffect(() => {
    ...
    netSend({ k: "mv", side: state.playerSide,
      type: localPiece.type, x: localPiece.x, rot: localPiece.rot, y: localPiece.y });
  ...
  React.useEffect(() => {
    ...
    netSend({
      k: "lk", side: state.playerSide,
      board: state.lockBoard, lines: state.lockLines,
  ...
  React.useEffect(() => {
    ...
    const finalize = () => setState(s => {
      if (!s.online || s.phase !== "playing") return s;
      if (s.iDied && s.oppDied) return { ...s, phase: "over", netResult: "draw" };
      if (s.iDied)  return { ...s, phase: "over", netResult: "lose", winner: s.playerSide === 1 ? 2 : 1 };
      return { ...s, phase: "over", netResult: "win", winner: s.playerSide };
    });
    // Both already dead -> resolve immediately (draw). Otherwise wait out grace.
    if (state.iDied && state.oppDied) { finalize(); return; }
  ...
  React.useEffect(() => {
    const active = state.online && state.phase === "playing"
                   && (state.paused || state.oppPaused);
    if (!active) {
  ...
  React.useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (axisLock !== "v") {
        if (!moved) {
          if (dx >=  FIRST_STEP_PX) { apply("right"); lastDx =  FIRST_STEP_PX; moved = true; }
          else if (dx <= -FIRST_STEP_PX) { apply("left"); lastDx = -FIRST_STEP_PX; moved = true; }
        } else {
          while (dx - lastDx >=  STEP_PX) { apply("right"); lastDx += STEP_PX; }
          while (dx - lastDx <= -STEP_PX) { apply("left");  lastDx -= STEP_PX; }
        }
      }
    };
    const onEnd = (e) => {
      ...
      if (axisLock === null && !moved) { apply("tap"); return; }
  ...
  React.useEffect(() => {
    ...
    const snap = {
      phase: state.phase, playerSide: state.playerSide,
      p1x: state.p1.x, p1rot: state.p1.rot,
      p2x: state.p2.x, p2rot: state.p2.rot,
      p1type: state.p1.type, p2type: state.p2.type,
      p1n: state.p1Next, p2n: state.p2Next,
      ...
      boundary: state.boundary, p1Lines: state.p1Lines, p2Lines: state.p2Lines,
      showSettings: showSettings, level: settings.level, paused: state.paused,
      sfxOn: settings.soundFX, hapOn: settings.haptics, vol: settings.volume,
    };
    driftLastSnap = snap;
  ...
  const selectSide = (side) => (e) => {
    ...
    console.log("[diag] selectSide", side, "| haptics:", settings.haptics,
      "| intensity:", settings.hapticIntensity, "| soundFX:", settings.soundFX,
      "| volume:", settings.volume);
    // Fire immediate test haptic so we know the bridge works at game start.
    const H = window.Capacitor?.Plugins?.Haptics;
    if (H) { try { H.impact({ style: "HEAVY" }); console.log("[diag] test impact OK"); } catch(e) { console.error("[diag] test impact FAILED:", e); } }
    navTo(() => setState(s => ({ ...s, phase: "playing", playerSide: side, aiLevel: settings.level })));
  };
  ...
  const disconnectRoom = () => {
    ...
    clearNetPieces();
    setState(s => ({ ...s, online: false, onlineStatus: "idle", onlineRole: null, opponentLeft: false, netResult: null }));
  };
  ...
  const connectToRoom = (code, opts = {}) => {
    ...
    if (opts.host) {
      ...
      setState(s => ({ ...s, onlineStatus: "waiting" }));
    } else {
      navTo(() => setState(s => ({ ...s, onlineStatus: "waiting", phase: "online" })), "slide");
    }
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.type === "role") {
        ...
        setState(s => ({ ...s, onlineRole: msg.role }));
        ...
        setState(s => {
          if (!s.online || s.phase !== "playing") return s;
          const piece = { type: msg.type, x: msg.x, rot: msg.rot, y: msg.y, lockPendingTs: null, lockResets: 0 };
          return msg.side === 1 ? { ...s, p1: piece } : { ...s, p2: piece };
        });
      } else if (msg.k === "lk") {
        ...
        setState(s => {
          ...
          const sh = shiftBoundary2P(
            { board: merged, boundary: s.boundary,
              p1: s.p1, p1Next: s.p1Next, p1NextNext: s.p1NextNext,
              p2: s.p2, p2Next: s.p2Next, p2NextNext: s.p2NextNext,
              lastGain: s.lastGain }, n1, n2);
          const adopted = { type: msg.piece.type, x: msg.piece.x, rot: msg.piece.rot, y: msg.piece.y, lockPendingTs: null, lockResets: 0 };
          const ovr = remote === 1
            ? { p1: adopted, p1Next: msg.next, p1NextNext: msg.nextNext }
            : { p2: adopted, p2Next: msg.next, p2NextNext: msg.nextNext };
          return {
            ...s,
            board: sh.board, boundary: sh.boundary,
            p1: sh.p1, p1Next: sh.p1Next, p1NextNext: sh.p1NextNext,
            p2: sh.p2, p2Next: sh.p2Next, p2NextNext: sh.p2NextNext,
            ...ovr,
            winner: sh.winner, phase: sh.phase, lastGain: sh.lastGain,
            p1Lines: s.p1Lines + n1, p2Lines: s.p2Lines + n2,
        ...
        setState(s => {
          ...
          if (s.phase === "over" && s.netResult === "lose") {
            return { ...s, oppDied: true, netResult: "draw" };
          }
          ...
          if (s.phase === "playing") {
            return { ...s, oppDied: true, resolveAt: s.resolveAt || Date.now() };
          }
          return s;
        });
      } else if (msg.k === "pause") {
    ...
    return (
      <div style={{
        ...
        {isWaiting && (
          ...
          }}>Waiting for opponent<span>.</span><span style={{animation:"ellipDot2 1.2s linear infinite"}}>.</span><span style={{animation:"ellipDot3 1.2s linear infinite"}}>.</span></div>
        )}
  ...
  if (phase === "start") {
    ...
    if (startTab === "2players") {
      ...
      return (
        <div style={{
          ...
          {/* Mode toggle at 448px */}
          <div style={{position:"absolute",left:0,right:0,top:448,display:"flex",justifyContent:"center"}}>
            {modeRow}
          </div>
          {/* Code display at 507px */}
          <div style={{
            position:"absolute", left:0, right:0, top:507,
          ...
          <div style={{
            position:"absolute", left:0, right:0, top:611,
            display:"flex", flexDirection:"column", alignItems:"center", gap:24,
            ...di(2),
          }}>
          ...
          <div style={{
            ...
            padding:"0 24px", fontSize:10, fontWeight:500,
            letterSpacing:2, color:"rgba(255,255,255,0.2)",
            pointerEvents:"none",
          }}>
    ...
    return (
      <div style={{
        ...
        {/* UP / OR / DN direction arrows at 514px */}
        <div style={{
          position:"absolute", left:0, right:0, top:514,
          ...
          >
            <span style={{fontSize:24,letterSpacing:"12px",marginRight:"-12px",opacity:0.5,fontWeight:300,color:"#fff"}}>↑</span>
            <span style={{fontSize:10,letterSpacing:"5px",marginRight:"-5px",opacity:0.5,fontWeight:600,color:"#fff",textTransform:"uppercase"}}>up</span>
          </div>
          <div style={{display:"flex",alignItems:"flex-end",height:64,paddingBottom:10}}>
            <span style={{fontSize:10,letterSpacing:"5px",opacity:0.3,fontWeight:600,color:"#fff",textTransform:"uppercase"}}>or</span>
          </div>
          <div
          ...
          >
            <span style={{fontSize:24,letterSpacing:"12px",marginRight:"-12px",opacity:0.5,fontWeight:300,color:"#fff"}}>↓</span>
            <span style={{fontSize:10,letterSpacing:"5px",marginRight:"-5px",opacity:0.5,fontWeight:600,color:"#fff",textTransform:"uppercase"}}>Dn</span>
          </div>
        </div>
        {/* DIFFICULTY at 618px */}
        <div
          ...
          onTouchStart={e=>e.stopPropagation()}
          style={{
            position:"absolute", left:0, right:0, top:618,
            display:"flex", gap:8, alignItems:"center", justifyContent:"center",
            minHeight:44, cursor:"pointer",
            ...di(2),
          }}>
          <span style={{fontSize:10,letterSpacing:"5px",opacity:0.5,color:"#fff",textTransform:"uppercase"}}>Difficulty:</span>
          <span style={{fontSize:10,letterSpacing:"5px",fontWeight:600,color:"#fff",textTransform:"uppercase"}}>{settings.level}</span>
        </div>
        {/* Music control removed - folded into Sound Fx (Settings). No standalone
            music control on the start screen when the game is not active. */}
        {/* Info + Gear icons at 754px */}
        <div style={{
          position:"absolute", left:0, right:0, top:754,
          display:"flex", gap:24, alignItems:"center", justifyContent:"center",
          opacity:0.5,
        ...
        {showSettings && (
          <SettingsScreen
            ...
            onUpdate={(key,val)=>{
              ...
              if (key==="level") setState(s=>s.online?s:{...s,aiLevel:val});
            }}
        ...
        {showInstructions && (
          <InstructionsScreen exiting={instructionsExiting} onClose={closeInstructions} />
        )}
        {/* TEST PROBE */}
        {TEST_PROBE && (
          <div ref={(el)=>{driftProbeEl=el;}} aria-live="polite" style={{
            position:"absolute",left:0,top:0,width:1,height:1,overflow:"hidden",
            opacity:0.01,color:"transparent",pointerEvents:"none",fontSize:1,
          }}>DRIFT;</div>
        )}
      </div>
    );
  ...
  return (
    <div style={{
      ...
      {/* (4.5) Drift parallax starfield -- NOT part of the Figma 145-3068
          spec; new prototype layer, only rendered when settings.drift is
          on. Two layers of randomly-scattered stars (STARFIELD_FAR_URL /
          STARFIELD_NEAR_URL, built once at module load -- see definition
          near DRIFT_TICK_MS) scroll the same direction as the drift shift,
          slower than the board (board moves CELL/DRIFT_TICK_MS =~9px/s;
          far layer ~1.4px/s, near layer ~3.6px/s), so the board reads as
          riding a surface instead of sliding over a dead backdrop. Each
          SVG is sized to the full frame and IS the seamless-loop repeat
          unit -- deliberately NOT a small repeating tile, which reads as
          a visible grid instead of a scattered field. */}
      {settings.drift && (
        <React.Fragment>
          <div style={{
            position: "absolute", inset: 0,
      ...
      {/* TEST PROBE: a11y bridge for XCUITest. Inert 1x1px off-screen node;
          its text content (set imperatively by writeDriftProbe) surfaces to
          the iOS accessibility tree. Not rendered when TEST_PROBE is false. */}
      {TEST_PROBE && (
        <div
          ref={(el) => { driftProbeEl = el; }}
          aria-live="polite"
          style={{
            position: "absolute",
            left: 0, top: 0,
      ...
      {/* Series score pips -- visible during play so user knows game 2/3 */}
      {(p1Wins + p2Wins) > 0 && phase === "playing" && (() => {
        const myW   = playerSide===1 ? p1Wins : p2Wins;
        const oppW  = playerSide===1 ? p2Wins : p1Wins;
        return (
          <div style={{
            position:"absolute", left:NEXT_X, top: `calc(${NEXT_Y - 36}px - max(20px, env(safe-area-inset-bottom)))`,
            width:NEXT_W, display:"flex", flexDirection:"column", alignItems:"center", gap:4,
          }}>
            <span style={{fontFamily:"'Inter',sans-serif",fontSize:8,fontWeight:600,letterSpacing:"2px",textTransform:"uppercase",color:"#fff",opacity:0.3}}>Series</span>
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              {[...Array(WINS_TO_WIN)].map((_,i)=>(
                <div key={i} style={{
      ...
      {state.opponentLeft && (phase === "playing" || phase === "over" || phase === "countdown") && (() => {
        ...
        return (
          <div style={{
            position:"absolute",inset:0,background:"#212223",zIndex:50,
            fontFamily:"'Inter',sans-serif",color:"#fff",overflow:"hidden",
          }}>
            <div style={dg}/>
            <BgVignette/>
            {/* "Connection" label at 419px */}
            <div style={{
              position:"absolute",left:0,right:0,top:419,
              display:"flex",justifyContent:"center",
              fontSize:16,fontWeight:500,letterSpacing:"8px",
            ...
            <div style={{
              ...
              {[0.1,0.2,0.3].map((o,i)=><span key={"dl"+i} style={{fontWeight:400,fontSize:12,letterSpacing:"2.4px",opacity:o}}>-</span>)}
              <ArrowR opacity={0.2}/>
              <span style={{fontSize:16,fontWeight:500,letterSpacing:"8px",marginRight:"-8px",opacity:0.5}}>Lost</span>
              <ArrowL opacity={0.2}/>
              {[0.3,0.2,0.1].map((o,i)=><span key={"dr"+i} style={{fontWeight:400,fontSize:12,letterSpacing:"2.4px",opacity:o}}>-</span>)}
            </div>
            {/* MENU at 599px */}
      ...
      {/* PAUSED screen -- Design: Figma 261:5916 */}
      {paused && !summary && phase === "playing" && (() => {
        ...
        return (
          <div style={{
            position:"absolute",inset:0,background:"#212223",zIndex:40,
            fontFamily:"'Inter',sans-serif",color:"#fff",overflow:"hidden",
          }}>
            <div style={dg}/>
            <BgVignette/>
            {/* "-> Paused <-" decorative row at 448px */}
            <div style={{
              position:"absolute",left:0,right:0,top:448,
              display:"flex",gap:16,alignItems:"center",justifyContent:"center",
              textTransform:"uppercase",
            }}>
              {[0.1,0.2,0.3].map((o,i)=><span key={"dl"+i} style={{fontFamily:"'Inter',sans-serif",fontWeight:400,fontSize:12,letterSpacing:"2.4px",opacity:o}}>-</span>)}
              <ArrowR/>
              <span style={{fontFamily:"'Inter',sans-serif",fontSize:24,fontWeight:500,letterSpacing:"12px",marginRight:"-12px",opacity:0.5}}>Paused</span>
              <ArrowL/>
            ...
            {state.online ? (
              <div style={{
                ...
                <span style={{fontSize:11,fontWeight:400,letterSpacing:"2px",opacity:0.3,textTransform:"uppercase",marginRight:"-2px"}}>you forfeit the game</span>
              </div>
            ) : (
              <div style={{
                position:"absolute",left:0,right:0,top:496,
      ...
      {state.oppPaused && !paused && !summary && phase === "playing" && (() => {
        ...
        return (
          <div style={{
            ...
            {/* Auto-forfeit countdown at 496px -- mirrors the pauser's PAUSED
                screen so the frozen player knows the wait is bounded and the
            ...
            <div style={{
              ...
              <span style={{fontSize:11,fontWeight:400,letterSpacing:"2px",opacity:0.3,textTransform:"uppercase",fontVariantNumeric:"tabular-nums",marginRight:"-2px"}}>in {forfeitLeft} sec, you win the game</span>
            </div>
      ...
      {/* GAME OVER overlays -- Design: Figma 258:6577 (Win), 267:6222 (Lose),
          267:6373 (Draw mid-set), 267:6673 (Win set-over), 267:6523 (Draw set-over).
          Shared layout: full-screen #212223 + grid, result row at 448px,
          "Best of 3" + pips at 496px, actions at 599px (gap 40px). */}
      {phase === "over" && (() => {
        const dg = {
          position:"absolute",inset:0,pointerEvents:"none",
          backgroundImage:
        ...
        const ResultRow = ({text, color, arrowOp=1, textOp=1, size=24, tracking=24}) => (
          <div style={{
            display:"flex",gap:16,alignItems:"center",justifyContent:"center",
            color:"#fff",textTransform:"uppercase",
          }}>
            {[0.1,0.2,0.3].map((o,i)=><span key={"dl"+i} style={{fontWeight:400,fontSize:12,letterSpacing:"2.4px",opacity:o}}>-</span>)}
            <ArrowR opacity={arrowOp}/>
            <span style={{fontFamily:"'Inter',sans-serif",fontSize:size,fontWeight:500,letterSpacing:tracking+"px",marginRight:"-"+tracking+"px",color,opacity:textOp}}>{text}</span>
            <ArrowL opacity={arrowOp}/>
            {[0.3,0.2,0.1].map((o,i)=><span key={"dr"+i} style={{fontWeight:400,fontSize:12,letterSpacing:"2.4px",opacity:o}}>-</span>)}
          </div>
        );

        // Shared pips: p1 red dots, p2 green dots, rest empty
        const Pip = ({color, filled}) => (
          <div style={{
            width:8, height:8, borderRadius:"50%", flexShrink:0,
        ...
        const Pips = ({p1, p2}) => (
          <div style={{display:"flex",gap:16,alignItems:"center"}}>
            {[...Array(3)].map((_,i)=>{
              if(i<p1) return <Pip key={i} color="#339900" filled/>;
              if(i<p1+p2) return <Pip key={i} color="#990000" filled/>;
              return <Pip key={i} filled={false}/>;
            })}
          </div>
        );

        // Shared screen wrapper
        const Screen = ({gameOverLabel, resultRow, p1, p2, primary, onPrimary, secondary, onSecondary}) => (
          <div style={{
            position:"absolute",inset:0,background:"#212223",zIndex:40,
            fontFamily:"'Inter',sans-serif",color:"#fff",overflow:"hidden",
            ...
            <div style={dg}/>
            <BgVignette/>
            {/* "GAME OVER" label (set-over only) */}
            {gameOverLabel && (
              <div style={{
                position:"absolute",left:0,right:0,top:419,
                display:"flex",justifyContent:"center",
                fontSize:12,fontWeight:600,letterSpacing:"6px",
            ...
            {/* Actions at 599px */}
            <div style={{
              position:"absolute",left:0,right:0,top:599,
