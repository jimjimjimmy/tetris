...
## Chapter 14: Capacitor iOS Setup (May 30)

**Commits: 38aec43**

The project had been a pure web app since day one. Adding Capacitor is the
first step toward shipping on the App Store and getting real native haptics
on iOS.

### Stack decisions

Capacitor 8.3.4 was installed (latest stable as of May 2026). Plugins:
- `@capacitor/haptics` - native iOS UIFeedbackGenerator (replaces navigator.vibrate)
- `@capacitor/status-bar` - full-bleed layout, no Safari chrome
- `@capacitor/splash-screen` - dark launch screen matching game aesthetic

No bundler was added. The project remains a single HTML file with CDN
React. Capacitor works fine with static web apps - it just needs a
`webDir` that it can copy into the iOS bundle.

### The webDir problem

The first attempt set `webDir: "."` (project root). Capacitor 8 rejects
this explicitly. The right answer was `webDir: "preview"` - but that
breaks the sound file paths.

The game's HTML file is at `preview/index.html`. Sound files were at
`assets/Sound fx/...` at the repo root. Relative paths in the HTML used
`../assets/Sound%20fx/...` (going up from preview/ to root, then into
assets/). With `webDir: "preview"`, Capacitor serves only the `preview/`
folder, so `../assets/` resolves to nothing.

**Fix**: Move `assets/` into `preview/assets/`. All relative paths in
`index.html` and `drift-test.html` become `assets/` (no `../`). This
works for both Capacitor (preview/ is served as root) and GitHub Pages
(preview/ is the published folder, assets are co-located).

### Haptics swap

The `_vibrate(pattern)` method inside the `haptic` object was updated to
check `window.Capacitor?.Plugins?.Haptics` before falling back to
`navigator.vibrate`. When running under Capacitor, it uses:

- Arrays (boundary gain double-tap) -> `notification({ type: "SUCCESS" })`
- Short durations (<= 15ms) -> `impact({ style: "LIGHT" })`
- Medium durations (16-50ms) -> `impact({ style: "MEDIUM" })`
- Long durations (> 50ms) -> `impact({ style: "HEAVY" })`

The intensity setting (`light/medium/strong`) shifts the style one step
up or down. All call sites (`pieceLock`, `lineClear`, `boundaryGain`,
etc.) are unchanged - they all route through `_vibrate`.

No import or bundler is needed because Capacitor injects the plugin
globals into the WKWebView at runtime.

### Placeholder assets

Icon and splash were generated with Python Pillow:
- `resources/icon.png` - 1024x1024, #0a0a0a background, white "D" (Helvetica 600pt)
- `resources/splash.png` - 2732x2732, #0a0a0a background, white "DRIFT" (Helvetica 200pt)

`@capacitor/assets generate --assetPath resources --ios` produced all
required iOS sizes (AppIcon.appiconset, Splash.imageset at 1x/2x/3x).
These are placeholder only - final design pass needed before App Store.

### Build result

`BUILD SUCCEEDED` on first attempt in Xcode 26.5 targeting iPhone 17 Pro
simulator. Three Capacitor plugins found and compiled via Swift Package
Manager (Package.swift auto-generated by `cap sync`).
...
# After any change to preview/index.html:
npx cap sync ios
# Then build/run in Xcode, or:
npx cap run ios

# Open in Xcode:
npx cap open ios

# Rebuild assets (after changing resources/icon.png or splash.png):
npx @capacitor/assets generate --assetPath resources --ios
npx cap sync ios
```

### Flags for physical device testing

The following need physical iPhone testing before App Store submission:
- Native haptics feel and timing (simulator has no Taptic Engine)
- Audio playback behavior (Web Audio API under WKWebView)
- Safe area / notch layout on actual device
- Splash screen duration and fade
