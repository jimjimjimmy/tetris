...
- `TetrisGame2P` - shared board with P1 (bottom half, floats UP, #B1B2B3) and P2 (top half, falls DOWN, #4a4a4a). Territorial tug-of-war via boundary shift.
  ...
  - Single-source-of-truth board (enum cells, wall row IS the boundary):
    Cells store enum values: `CELL_EMPTY=0`, `CELL_P1=1`, `CELL_P2=2`, `CELL_WALL=3`. The boundary is not a separate variable -- it is a physical row of `CELL_WALL` values inside `board`. `isValid2P(cells, board)` rejects any move where `board[r][c] !== CELL_EMPTY`, so the wall row blocks pieces identically to walls/floor/ceiling. On row clears, `moveWall2P(board, oldBdy, newBdy)` relocates the wall row in `board` and clears the wall's path (the wall acts like a physical barrier that pushes through whatever is in its way). Helpers: `initBoard2P`, `isValid2P`, `lockPiece2P`, `moveWall2P`. Render maps enum to grayscale colors via `CELL_TO_COLOR_2P` -- only #B1B2B3 (P1), #4a4a4a (P2), and #7a7b7c (wall) ever appear on cells.
    P1 spawn formula updated to `P1_VIEWPORT_H - pieceMaxDr(type, 0)` (= 20 - max_dr) so P1 travel matches P2 travel given the wall row now occupies one row of what used to be a virtual seam.
