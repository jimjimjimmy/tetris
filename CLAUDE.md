...
- `TetrisGame2P` - shared board with P1 (bottom half, floats UP, #B1B2B3) and P2 (top half, falls DOWN, #4a4a4a). Territorial tug-of-war via boundary shift.
  ...
  - Hard-wall boundary: the boundary acts as a solid collision barrier for BOTH active and locked pieces. `isValid` blocks active pieces from crossing. After each boundary shift, `enforceBoundary2P(board, bdy)` sweeps the grid and evicts any locked cells whose color identifies them as the opponent's piece in the newly-claimed territory. So when the line moves, the loser's stack adjacent to it is wiped from the rows it now owns.
