...
- `TetrisGame2P` - shared board with P1 (bottom half, floats UP, #B1B2B3) and P2 (top half, falls DOWN, #4a4a4a). Territorial tug-of-war via boundary shift.
  ...
  - Hard-wall boundary (preventive, not reactive). Two preventive checks ensure no cell can ever exist on the wrong side:
    (a) `isValid` rejects any active-piece cells at r<boundary (P1) or r>=boundary (P2). The boundary is treated identically to the walls/floor/ceiling -- every rotate/slide/fall is gated by it.
    (b) After computing the shift accumulator drain, the boundary is clamped so it cannot move past existing cells: `newBdy = max(newBdy, maxP2Row + 1)` and `newBdy = min(newBdy, minP1Row)`. The boundary line treats locked cells as obstacles -- it can't slide through them. If the opponent has stacked all the way to the boundary, the boundary can't advance until they clear their adjacent row.
    Result: no sweep needed. By construction, every cell that ever exists in `board` is on its rightful side of `boundary`.
