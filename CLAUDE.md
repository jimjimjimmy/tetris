...
- `TetrisGame2P` - shared board with P1 (bottom half, floats UP, #B1B2B3) and P2 (top half, falls DOWN, #4a4a4a). Territorial tug-of-war via boundary shift.
  ...
  - ROWS_2P=44 (the 2P-specific row count). EVEN, so boundary = ROWS_2P / 2 = 22 sits at the exact midpoint and both players own identical territory of ROWS_2P/2 = 22 rows each. No spawn buffer -- every row is visible and playable. (The legacy 1P standalone games keep ROWS=33 with their old layout; ROWS_2P is a separate constant just for TetrisGame2P.)
  - BDY_2P = ROWS_2P / 2 = 22 is the INITIAL boundary only. Actual boundary is dynamic state, tracked as `boundary` in component state.
  ...
  - Win condition: `boundary <= 0` -> P1 WINS (P2 squeezed out). `boundary >= ROWS_2P` (44) -> P2 WINS (P1 squeezed out). GAME OVER overlay shows "P1 WINS" / "P2 WINS" with "territory claimed" subtitle.
